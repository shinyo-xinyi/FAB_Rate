...
def get_data(cur_item, crawler):
    ...
    for item in item_dictionary:
        if cur_item["product"] == item["product"]:
            if cur_item["reviews"] == item["reviews"]:
    ...
    print("No record found! Running crawler...Total reviews number is " + str(cur_item["reviews"]))
    url = get_url(cur_item["product"])
    cur_item = crawler.get_reviews_result(url, cur_item)
    # record the new item
    item_dictionary.append(cur_item)
    print("new_item:")
    print(cur_item)
    write_db(cur_item)
    return cur_item
...
def get_dic(origin_url):
    ...
    url = get_url(pid)
    crawler = Crawler(url)
    reviews = crawler.get_reviews_number()
    # initialize the current item
    cur_item = {"product": pid, "reviews": reviews, "pos": 0, "neu": 0, "neg": 0, "score": -1, "keywords": []}
    if reviews == 0:
        # the product has no reviews
        return cur_item
    # crawl the reviews and do NLP to get the result in a dictionary
    dic = get_data(cur_item, crawler)
    return dic